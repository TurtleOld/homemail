...
export default function MailLayout({ children }: { children: React.ReactNode }) {
  ...
  const currentMailUrlState = useMemo<MailListUrlState>(() => ({
    folderId: selectedFolderId || undefined,
    search: searchQuery,
    quickFilter,
    presentation: conversationView ? 'conversation' : 'flat',
  }), [selectedFolderId, searchQuery, quickFilter, conversationView]);
  ...
  const bulkActionsToolbar = selectedIds.size > 0 ? (
    <div
        ...
        <section className="flex min-w-0 flex-1 flex-col" aria-label={t('workspaceLabel')}>
          {(!isMobile || !selectedMessageId) && (
            <header className="mail-panel-surface flex min-h-16 flex-shrink-0 items-center gap-4 border-b border-border px-4 py-2 max-md:min-h-14 max-md:px-3">
              ...
              />
              {!selectedMessageId && (
                <div className="flex flex-shrink-0 items-center gap-2 max-md:hidden">
                  <QuickFilters
                  ...
                  >
                    <MessageSquare className="mr-1 h-4 w-4" strokeWidth={1.8} />
                    <span>{t('threads')}</span>
                  </Button>
                </div>
              )}
                ...
                <DropdownMenu>
                  <DropdownMenuTrigger asChild>
                    <Button
                      variant="ghost"
                      size="icon"
                      className="hidden h-9 w-9 flex-shrink-0 rounded-lg shadow-none max-md:flex"
                      title={t('listOptions')}
                  ...
                  <DropdownMenuContent align="end" className="w-64 p-2">
                    ...
                    <DropdownMenuItem
                      ...
                      className="mt-1"
                    >
                      <RefreshCw className={cn('mr-2 h-4 w-4', isRefreshing && 'animate-spin')} strokeWidth={1.8} />
                      {t('refresh')}
                    </DropdownMenuItem>
                    <DropdownMenuItem onClick={handlePresentationChange}>
                      <MessageSquare className="mr-2 h-4 w-4" strokeWidth={1.8} />
                      {conversationView ? t('switchToNormal') : t('switchToThread')}
                    </DropdownMenuItem>
                  </DropdownMenuContent>
                </DropdownMenu>
              )}
