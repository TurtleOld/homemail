...
Status: Phase 0 through Phase 6 complete (Phase 5 and Phase 6 use the scope narrowed by ADR 0008/0009; see the corresponding decision sections); Phase 7 flag/dead-code cleanup complete locally, not yet deployed
...
### Increment 2: OIDC identity linking wired into the real OAuth callback

This increment does exactly what Increment 1's "next safe step" specified: it wires `validateOidcIdToken` and `bootstrapAdministratorIfConfigured` into the real `/api/auth/oauth/authorize` and `/api/auth/oauth/callback` routes, still entirely behind `HOMEMAIL_FEATURE_FAMILY_IDENTITY`, without switching session authorization away from the existing access-token-based legacy login.

Before starting this increment, a planning gap surfaced and was resolved with the user: HomeMail's own provisioning of family members and mailboxes will need to authenticate to Stalwart's Management API itself (via `StalwartAdminAdapter`), and that credential must be a dedicated Stalwart API-key service credential (`STALWART_ADMIN_API_KEY`), never a human's personal Stalwart login, and never the same thing as `HOMEMAIL_BOOTSTRAP_ADMIN_EMAIL` (which only decides which verified sign-in becomes the HomeMail administrator role). This is documented for operators in the new `docs/FAMILY-IDENTITY-FIRST-RUN.md`, which also lays out the exact first-run sequence: Stalwart's own fallback administrator creates the API key in Stalwart Web Admin, the operator sets both environment variables, and only then does the real family-member provisioning (a later increment) become possible. `STALWART_ADMIN_API_KEY` is documented in `.env.production.example` but not yet consumed by any code path.

#### Added

- `lib/oidc-jwks-provider.ts`: `HttpJwksProvider`, a minimal `OidcJwksProvider` implementation that fetches a JWKS document over HTTP(S), validates each entry has at least a string `kty` (dropping malformed entries rather than passing them to the validator), and short-term caches the result (10-minute TTL, injectable clock for tests).
- `lib/oauth-pkce.ts`: added `generateNonce()` (same 128-bit entropy construction as the existing `generateState()`) and an optional `nonce` parameter on `buildAuthorizationUrl()`, appended to the authorization URL only when provided.
- `lib/oauth-state-store.ts`: `OAuthState` gained an optional `nonce` field, `storeOAuthState()` accepts and persists it, and `consumeOAuthState()` now returns `{ codeVerifier, nonce } | null` instead of a bare string — its only caller (the callback route) was updated in the same change.
- `lib/oauth-discovery.ts`: `OAuthDiscoveryResponse` gained an optional `jwks_uri` field (previously undeclared, even though Stalwart's discovery document includes it — confirmed in the Phase 0 evidence above at `https://auth.pavlovteam.ru/auth/jwks.json`).
- `app/api/auth/oauth/authorize/route.ts`: when `HOMEMAIL_FEATURE_FAMILY_IDENTITY` is enabled, requests the `openid` scope in addition to the existing JMAP scopes, generates a nonce, and stores it alongside the PKCE verifier. When the flag is disabled, the authorization URL and stored state are byte-for-byte what they were before this increment.
- `app/api/auth/oauth/callback/route.ts`: when the flag is enabled and the token response contains a string `id_token`, validates it via `validateOidcIdToken` (issuer and jwks_uri from the same discovery response already used for the token endpoint, audience set to `OAUTH_CLIENT_ID`, nonce from the consumed state) and then calls `bootstrapAdministratorIfConfigured` with the validated issuer/subject/email. This entire block is wrapped in its own `try/catch`: any failure (network, malformed token, signature, issuer/audience/nonce mismatch) is logged as a warning and swallowed, never surfacing an error to the user or altering the existing access-token session-creation path that follows it. When the flag is disabled, or the provider did not return an `id_token`, none of this code executes.

#### Verification performed

- `npx tsc --noEmit`: passed.
- `lib/__tests__/oidc-jwks-provider.test.ts` (5 tests): valid-entry parsing, malformed-entry filtering, non-2xx status handling, cache-hit within TTL, cache-miss after TTL.
- `lib/__tests__/oauth-authorize-family-identity.test.ts` (2 tests): confirms the authorization URL and stored state contain no `openid` scope and no `nonce` when the flag is disabled, and both are present and consistent with each other when enabled.
- `lib/__tests__/oauth-callback-family-identity.test.ts` (4 tests): confirms `validateOidcIdToken`/`bootstrapAdministratorIfConfigured` are never called when the flag is disabled; confirms both are called with the expected arguments when enabled and a valid `id_token` is present, while the legacy `createSession` call still completes; confirms the legacy sign-in still completes and no error is surfaced to the redirect URL when ID-token validation throws; confirms no validation is attempted when the provider omits `id_token` even with the flag enabled. The "legacy sign-in continues after a validation failure" test was verified to actually catch a regression by temporarily short-circuiting the callback's linking branch and confirming that specific test failed, then restoring the real code and confirming all four tests passed again.
- Full `npx vitest run --maxWorkers=1 --no-file-parallelism`: 45 files, 243 tests passed.
- `npx eslint` over every new and changed file: passed without errors or warnings.

#### Known limitations and next safe step

- The legacy access-token login path is structurally unchanged: session creation, JMAP account resolution, and `user_accounts.json` bookkeeping still happen exactly as before, regardless of whether family-identity linking succeeds, fails, or is skipped.
...
- No instance-administration surface exists in HomeMail; Stalwart integration, backup/restore, DKIM, TLS, and security policy remain the server operator's direct responsibility through Stalwart Web Admin and operator tooling, not through HomeMail. (Read-only monitoring is revisited below — ADR 0009 narrows this further and adds it back as a display-only capability.)
- This "next safe step" is superseded below — a real `StalwartAdminMutationTransport` is no longer planned. See ADR 0009.

### ADR 0009: Stalwart administration and family provisioning permanently out of scope (2026-07-19)

Continuing to design the next increment (a real `StalwartAdminMutationTransport`), the user asked what Stalwart 0.15's REST Management API actually exposes before committing to build against it, since public documentation mixes 0.15 and 0.16 API shapes and cannot be trusted alone (confirmed directly: fetching what appeared to be current API docs returned 0.16-style JMAP management objects — `Domain/set`, `sysDomainCreate` permissions — that do not exist on the running 0.15.3 server; several guessed 0.16-shaped `/api/manage/*` paths all 404'd against it).

With the operator's explicit permission, the actual 0.15.3 REST API was queried directly against the local `homemail-stalwart-dev` container (read-only discovery, then one throwaway principal created and deleted to confirm the full CRUD lifecycle). Confirmed real behavior:

- `/api/principal` is the single endpoint for both domains and mailboxes (`type: "domain"` vs `"individual"`), supporting `GET` (list and by-name), `POST` (create), `PATCH` (field-level update via `[{action, field, value}]`), and `DELETE` — all verified working, including a full create → update quota → delete → confirm-gone cycle.
- `/api/queue/messages` and `/api/queue/reports` expose the outbound mail queue and report backlog (empty on the dev instance, but confirmed to respond with a real, parseable shape).
- `/api/oauth` manages OAuth clients (POST with a body; this is what Stalwart Web Admin's own OAuth-client screen uses).
- `/api/account/auth` and `/api/account/crypto` expose 2FA/app-password and encryption state for the *authenticated* principal only — not administrative.
- `/api/settings/*` (e.g. `/api/settings/group`, `/api/settings/list?prefix=...`) is a broad, unredacted view of the entire running configuration, including the fallback administrator's bcrypt password hash and every domain's private DKIM keys in plaintext. This was observed directly during the investigation (dev-only secrets, confirmed by the operator to be temporary and already slated for rotation) and is the concrete evidence behind ADR 0009's requirement that this surface never be reachable through any HomeMail-held credential.
- `/api/telemetry` is Enterprise-only and unavailable on the community edition Stalwart HomeMail targets.
- Several guessed endpoints (`/api/domain`, `/api/mailbox`, `/api/dkim`, `/api/dns`, `/api/store`, `/api/oauth-client`, `/api/api-key`) do not exist as separate paths on 0.15.3; the functionality either lives under `/api/principal`/`/api/oauth` or was not found.

Having confirmed that account/domain provisioning is technically buildable through `/api/principal`, the user reconsidered whether HomeMail should build it at all, reaffirming the deployment model behind ADR 0008: one small trusted family, one operator who already holds full Stalwart administrator access through Stalwart Web Admin. The decision: HomeMail never creates, updates, or deletes any Stalwart resource. All server administration — mailbox/family-member lifecycle, domains, DKIM, TLS, security policy — remains a direct, permanent Stalwart Web Admin responsibility. HomeMail's only Stalwart-facing capability going forward is **read-only monitoring** (mail queue status, delivery/report backlog, and similar display-only signals), authenticated with a narrowly scoped `STALWART_ADMIN_API_KEY` that must never be granted `/api/settings/*` access.

This is recorded in `docs/adr/0009-keep-stalwart-administration-out-of-homemail.md`, which formally and fully supersedes ADR 0003 (activation links) and ADR 0004 (recovery links) — both were built entirely around a HomeMail-driven provisioning scenario that no longer exists. Both source ADR files are marked Superseded, pointing to ADR 0009. ADR 0002's `StalwartAdminAdapter` boundary concept is unaffected in principle but narrows in practice to a small read-only contract; its currently-declared mutation operations (`principal.create/update/suspend/delete`, `mailbox.create`, `credential.update`, `oauth.revoke`) have no planned consumer and must not be built against without a fresh ADR revisiting this decision.
...
### Gate re-check finding: backup evidence age

Before starting this session's work, the Phase 0-6 exit gates were reviewed against the plan's release gates ("Production inventory, backup evidence, isolated restore evidence, and last-known-good rollback commands are current before any stateful phase"). The production backup, off-host copy, checksum validation, isolated restore rehearsal, and rollback-command evidence recorded under Phase 0 are dated 2026-07-18. Three production deploys landed after that date and before this session: Phase 4 (protected message content and remote image fetching enabled), Phase 5 (read-only Stalwart monitoring shipped), and Phase 6 (Settings route/localization changes). This is recorded here as an explicit gate finding, not silently treated as satisfied and not treated as blocking: this session performed no production mutation, and the operator's own Phase 7 runbook already requires a fresh backup as a precondition of any future production deploy step, which supersedes relying on the 2026-07-18 evidence for that purpose. The last-known-good image digests and rollback commands recorded in Phase 0 remain the reference point but have not been re-verified against the current running production images as part of this session.

### Explicit scope departure from the original Phase 7 prompt

`docs/MAIL-REDESIGN-PHASE-PROMPTS.md`'s Phase 7 prompt requires: "Не удаляй compatibility code или legacy data в cutover release. Cleanup является отдельным последующим release после observation window, fresh backup, dry-run и explicit approval." The operator explicitly and knowingly overrode this rule for this session's work, choosing to remove the redesign feature-flag scaffolding and its flag-off legacy branches, plus fully dead Phase 1/5 identity scaffolding, inside Phase 7 itself rather than deferring it to a separate approved cleanup release. This is recorded here as a deliberate, operator-directed departure from the plan's normal expand/enable/contract discipline — not a silent violation and not a new default for future phases. Per the operator's explicit choice, this departure is recorded only as a log entry, not as a new ADR: the criteria for an ADR (hard to reverse, surprising without context, the result of a genuine trade-off between real alternatives) were judged not to be met for a flag/dead-code removal of already-production-accepted behavior, as distinct from the earlier ADR 0008/0009/0010 product-scope decisions.

### What was removed

All 7 redesign/identity feature flags were removed from `lib/feature-flags.ts` (deleted entirely once empty), across four commits:

1. **`1e90efe`** — the fully-dead Phase 1/5 family-identity cluster, none of which had any caller outside itself and its own tests: `lib/legacy-identity-adapter.ts`, `lib/authorization-policy.ts`, `lib/configuration-scope.ts`, `lib/family-identity-store.ts`, `lib/home-identity.ts`, and their test files (`lib/__tests__/authorization-policy.test.ts`, `lib/__tests__/domain-boundaries.test.ts`, `lib/__tests__/family-identity-store.test.ts`). This also removed the three flags that gated nothing (`identityFoundation`, `authorizationPolicy`, `oidcIdentityValidation`) — these were never wired into any live session, route, or UI path per the Phase 1 log.
2. **`6749a99`** — `productShell`. Enabled in production and operator-accepted since Phase 2. Collapsed every `productShellEnabled` ternary in `app/[locale]/login/page.tsx` (including the three `Legacy*Icon` SVG helper components, now unreachable) and `app/[locale]/contacts/page.tsx` down to the redesign branch. Removed the prop from `app/providers.tsx`, `app/[locale]/layout.tsx`, and `components/product-shell/shell-feature-context.tsx`; removed the now-dead `enabled` gate from `components/product-shell/route-aware-shell.tsx`, since the route-aware shell wrapper is unconditional.
3. **`01c4e1e`** — `listFirstMail`. Enabled in production and operator-accepted since Phase 3. Collapsed 30+ conditional sites in `app/[locale]/mail/layout.tsx` (state initialization, URL-sync effects, folder/search/presentation handlers, and JSX for the sidebar, header, list pane, and reader pane), removing the entire legacy three-pane split-view layout and its resizable-divider state/handlers (`messageListWidth`, `isResizing`, `resizeStartRef`, `handleMouseDown`), which had no remaining caller once the divider JSX was removed. Made `/{locale}/mail/messages/{messageId}` and the `Thread/get` API route (`app/api/mail/threads/[threadId]/route.ts`) unconditionally available instead of 404ing when the flag was off.
4. **`3cf8c9b`** — `protectedMessageContent` and `remoteImageFetching`. Both enabled in production and operator-accepted since Phase 4. Collapsed the flag ternaries in `components/message-viewer.tsx` (the CSP meta tag, `effectiveAllowImages`, and the `iframeSrcDoc` memo dependency) and the two API routes that build message/thread responses (`app/api/mail/messages/[id]/route.ts`, `app/api/mail/threads/[threadId]/route.ts`) and the image resource route (`app/api/mail/resources/image/[token]/route.ts`, which dropped its two `isRedesignFeatureEnabled` guard checks, leaving its actual security logic — signed token validation, IP/DNS/redirect/MIME checks — untouched). Since `effectiveAllowImages` is now permanently `false`, the legacy manual "Show images" opt-in banner and its `localAllowImages`/`hasRemoteImages` state became dead and were removed, along with the `allowRemoteImages` prop that threaded through `MessageViewer`, `ConversationReader`, and an already-inert `allowRemoteImages` state in `mail/layout.tsx` (its setter was never called even before this session). This matches the Phase 4 log's own note that protected mode already ignores the legacy direct-loading opt-in. `lib/feature-flags.ts` and `components/product-shell/shell-feature-context.tsx` were deleted entirely in this commit, since `remoteImageFetching` was the last remaining flag.

### Rollback posture for this change

No new runtime kill switch exists for product shell, list-first mail, protected message content, or remote image fetching after this change — flipping an environment variable can no longer revert to the legacy rendering paths, because those paths no longer exist in the codebase. Rollback for a regression discovered after this code deploys uses the existing Phase 0 rollback triggers (authentication/sign-in failure, OIDC discovery breakage, Stalwart storage/JMAP/SMTP/IMAP unavailability, missing accounts/mail, inability to complete smoke checks within the maintenance window) plus one additional trigger specific to this deploy: any regression that surfaces only after the flag-off branches are gone requires the Phase 0 image-only rollback procedure (reverting to the last-known-good HomeMail image), not a flag flip. No separate extended observation window was defined beyond the standard rollback-trigger monitoring, since all four collapsed flags were already stable, operator-accepted, and running in production before this cleanup — this change removes now-unreachable code paths around already-proven behavior, it does not introduce new behavior.

### Explicit operator decision to skip a fresh backup before this deploy

The operator explicitly chose not to take a fresh backup before this specific future deploy step, reasoning that this change is a code-level refactor that removes dead code paths and does not directly mutate `/app/data` or Stalwart state, so an image-only rollback is sufficient if a regression surfaces. This is recorded as a deliberate, informed choice made when asked, not an oversight or a silent skip of the plan's normal backup discipline.

### What remains out of scope for this session

No production step was performed: no backup, no deploy, no `.env` change on the production host, no smoke check against live Stalwart or production HomeMail. All four commits above are local repository changes only, verified with `npx tsc --noEmit` (clean after every commit), the full `npx vitest run --maxWorkers=1 --no-file-parallelism` suite (41 files / 223 tests passing after the final commit, down from 45 files / 245 tests before Phase 7 cleanup started, reflecting the removed dead-code test files and the flag-branch test cases that no longer apply), `npm run lint` (0 errors; the same 11 pre-existing warnings in unrelated files recorded throughout this log remain, confirmed identical by comparing against a stash of the pre-Phase-7 tree), and `npm run build` (passed, all routes including `/[locale]/mail/messages/[messageId]` and `/api/mail/threads/[threadId]` compiled). Deployment, `.env` flag removal on the production host, and post-deploy smoke checks remain separate, explicitly operator-approved actions.

### Smoke-check plan for when deploy is later approved

In addition to the full regression suite already run locally above, the operator should specifically confirm that the legacy session/storage compatibility reads in `lib/storage.ts` (the `STORAGE_KEY_RE` pattern and `readStorage`/`writeStorage` primitives, which resolve real email-shaped `accountId` keys such as production's `admin@rem.ru` per the Phase 4 storage-key defect fix) continue to function correctly after this deploy. This is existing, still-load-bearing storage compatibility code that is unrelated to and was not touched by this cleanup — it must not be confused with the deleted `lib/legacy-identity-adapter.ts`, which was dead scaffolding for a different (family-identity) concern that ADR 0008/0009 already superseded and which had no live caller before its removal.

### Known limitations and next safe step

- The 2026-07-18 Phase 0 backup/restore/rollback evidence has not been re-verified against the production images actually running today; this is the gate finding recorded above.
- This session's four commits remain uncommitted to any remote and undeployed; `git log` shows the repository nine commits ahead of `origin/main` as of this work.
- A small number of pre-existing untracked files (`docs/MAIL-REDESIGN-PHASE-PROMPTS.md`, `docs/MAIL-REDESIGN-PHASE0-RUNBOOK.md`, `docs/MAIL-REDESIGN-PLAN.md`, the four Phase 0 evidence scripts) were present as untracked working-tree files at the start of this session and remain untracked; they were not part of this session's changes and were deliberately left alone rather than swept into a commit via a broad `git add`.
- Phase 7 is not complete. What remains: operator-approved production deploy of these four commits, the smoke checks described above, and a stable observation period per the plan's Phase 7 exit gate ("the redesigned product is stable for the agreed observation period, rollback is no longer required for normal incidents") before this phase can be declared closed.
